                                ...
                                uint8_t address_pins)
{
    ...
    /* Calculate 8-bit address based on Pins (A0-A5)
       0x40 is base. Pins are limited to 6 bits (0x3F) */
    ...
    if (dev->invrt) {
        mode2 |= MODE2_INVRT; /* Apply inversion */
    }
                                                ...
                                                uint8_t *duty_cycles)
{
    ...
    /* Burst Write starting from LED0_ON_L (0x06)
       Write 64 bytes sequentially. The auto-increment handles the register addresses.
